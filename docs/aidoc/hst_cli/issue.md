...
- `clamp_issue_limit` — issue list 的 limit 钳制（1 至 100；上限源 = 总台 REQ-057 服务端契约，
- `file_issue` — 提交面（new）：title trim 后 1 至 200、body 至多 20000、version 至多 40
- `issue_list_saturated` — issue list 饱和判定（#52 同型修，评审 G1）：返回条数不少于钳制后
- `issue_list_truncation_hint` — issue list 饱和提示行（#52 同型修，#53 补翻页出口）：返回条数打满
- `list_issues` — # Errors
...
- `IssueListPage` — issue list 的一页回执（#53）：`rows` 是本页条目（新到旧）；

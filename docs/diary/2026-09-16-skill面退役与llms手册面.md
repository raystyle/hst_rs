...
7. **二轮快核与三轮 CONFIRM**：二轮回 F 一（三处权威文本口径滞后：R002 加 ADR 加 CHANGELOG 仍写「用户级 `~/.claude/skills/` 两名」而行为已扩四根）加 G 两（AGENTS 跨仓行 dev-evo 旧 D 盘位、REQ-001 trace 双死路径），落 2ab4e00（六文件六行：三句口径统一、跨仓行换 `~/repos/project-evo`、trace 修现位、测试名随实改）；三轮 CONFIRM 放行（codex 顺手实跑 trace 命令验证路径活、版本面复核全仓无 1.3.0 残留、笔数勘误五笔非六）。
8. **v2.0.0 封版执行**：五笔推 main（c4cba3f 至 2ab4e00，run 35117207324 六岗绿）；tag v2.0.0（run 35117506746 六岗绿）；六资产 digest 三方对账全等（win a2a8fe80、linux 2b7c8686、mac df83e5cd，本地算加边车加 API 三方一致）`[实证: 2026-09-16 sha256sum 对照]`；镜像 hst/stable 三边车 curl 200 且 digest 与 Release 全等 `[实证: 2026-09-16 env.ohmygh.com/hst/stable/]`；装机侧 `hst self update --stable` 1.3.0 至 2.0.0 滚版成功（update.ok=true），装机态终验 `hst --llms` 出手册、`hst skill` 报 unrecognized、`hst init` 幂等零退役行 `[实证: 2026-09-16 ~/.local/bin/hst]`；herdr 回执总台 ohmycloud（catalog 滚版对端接）。

...
- [x] 传输零新依赖：路径解析 HERDR_SOCKET_PATH 到 HERDR_SESSION（`~/.config/herdr/sessions/<名>/herdr.sock`）到缺省 `~/.config/herdr/herdr.sock`；Windows `\\.\pipe\` 前缀映射；本通道不注入任何 token（本地 socket 权限即边界；评审 F2 回填：工件口令情报非 herdr 机制）；一连接一请求短连接
...
- [x] 实弹：真 socket ping 探活（python 直连）加向本会话格真派发（评审 F1 修复后布防，回执闭环见 diary）

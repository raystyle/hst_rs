...
- [x] artifact 族：`artifact publish`（name 加 kind 十五种加 digest 加 version 加 git_range 加 deps[] 加 note；summary 与 outcome 随 v0.1.2 恢复、git_sha 随 v0.1.3 恢复（publish_full 十参，尾项销））、`artifact attest --type`（收口后三验证型）、~~`artifact promote`~~ 随收口令移除、`artifact list --current --env`（只读面免钥）

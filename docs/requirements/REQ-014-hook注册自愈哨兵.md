...
- [x] 节流读侧走 `[IO.File]` 直取 mtime（dotfile 在 Unix pwsh 带 Hidden 属性，`Get-Item` 不带 `-Force` 取不到，读侧失败 fail-closed 不自愈，评审 F1）；stamp 首创建独占（并发窗格败者视作他人刚自愈，评审 G2）
- 已知边界一：探针是「任一 hst 注册标记在册即过」的启发式，部分丢失（如仅 PostToolUse 被摘、SessionStart 条目幸存）不触发自愈；实测事故形态是整段抹除，哨兵覆盖该形态

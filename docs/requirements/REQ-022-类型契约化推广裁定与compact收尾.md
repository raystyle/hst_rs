...
status: implemented
priority: should
trace: compact 单测三件（threshold 变体六断言改枚举形全绿）加推广面实测盘点（String 错误计数加 bool 元组全仓扫描加 Json 载体审查，2026-09-26 diary 流水 12）
---
...
- [x] 推广面实测盘点：全仓 String 错误计数（deploy 19 加 yolo 14 加 statusline 13 加 update 11 等）、bool 元组返回扫描（唯一残点 = compact threshold_approx）、Json 载体审查（外部属主长 schema 面名单）

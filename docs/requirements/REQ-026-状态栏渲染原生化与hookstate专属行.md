...
- [x] 第5行 hookstate 专属行：statusline.toml 新键 `segments5`、`DEFAULT_SEGMENTS5 = ["hookstate"]`、模板 `{icon}  {events} {state}` 加 ascii 变体（图标后双空格对齐 hst 段形，用户令 2026-09-26「没有2个空格」）、图标 U+F0F1、状态色同 hst 段四色；`{events}` = 注册面实读的 hst 状态 hook 挂载事件清单（用户令「hook了什么事件」：按 agent 定位注册文件 claude settings.json 加 codex hooks.json 加 grok ohmyagents-state.json 三家 JSON 同构加 kimi config.toml [[hooks]]，ours 判定 = command 含 hst-state 干 stem 与 REQ-014 哨兵同源，按部署事件序稳定排列，外来 hook 不混入，读不出回落泛称 hook）；出行门 = 状态文件在场或有挂载事件任一（评审快核 G1：刚装未触发的空窗期不隐清单，状态缺报回落 unknown；已知背离在册：注册面已清而 state 文件遗留时显示泛称 hook 不反映卸载）；双缺整行隐藏（零噪声）；pwsh 侧该段空块（弃用期不出，行隐）

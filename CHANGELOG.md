...
- **cli-docs 采纳轮 v2.3.0**（总台 2026-09-18 全仓采纳令，标准 = evo-adr cli-docs 技能）：帮助面节序对齐（根头行 `hst@<版本>` 一句定位经 help_template 从载体 manifest 注入、Options 与 Global Options 分节经 help_heading 派生、尾行 `--llms` 发现指引；叶形 Arguments、Options、Global Options 节序随派生到位）；输出协议补 `--filter-output <keys>`（必选旗标七件本仓五件在册：json 信封 data 键路径过滤，括号感知逗号分隔、点号嵌套、单下标取元素可续导航、多下标集成数组且为终点，响错按类分道不静默截断，走 stderr 单行加退出 1；kv 与 jsonl 配对直接报错；`--schema` 与 `--full-output` 与类型化 CTA 等余件登记 REQ-011 不冒领）加信封 meta 补 duration_ms；`--llms` 手册补读序与退出码两节加 --json 独立行（72 行仍小于 120，退出码 0/1/2 口径入册）；残破描述修复 13 处六型（`--format`、`--offset`、hook status、trace blocks、diagnose cache 等「的X面（细则见）」样）；漂移守卫集成测试（机器形遍历命令树，逐命令 --help 提取长旗标，断言全部出现在手册，防新增参数漏登记）；README 甲面补徽章三枚（CI、Release、MIT）加特性一览七条加环境变量缺省列加 sha256 边车校验行加许可尾节；协议余件（--schema、类型化 CTA、细分错误码）立 REQ-011 评估不静默丢。fmtio 单测四件加 cli 集成五件新，198 加 39 绿、clippy 基线 13 无新增、aidoc strict 0。
- **selfupdate 家族统一标准对齐 v2.4.0**（总台家族自更新统一标准轮 2026-09-18 用户裁「采纳推广」，权威 = browse-rs REQ-005 加 build-release 公共契约第六节；ADR-0008 替代 ADR-0004、REQ-013）：缺省读序翻向（`HST_MIRROR` 未设 = 默认基址 env.ohmygh.com 镜像段优先，latest 通道落 stable 滚动段、dev 通道落 dev 滚动段同读序，任一步网络类失败整对回落 GitHub 官方不回环；设值 = 基址覆盖同读序、空串 = 全关；kv 标记 fallback-default 形退役换 default-first 形，D48 回退守卫函数随结构化不回环退役）；GitHub 官方腿补 digest 锚硬校验（API digest 归一优先、缺省回落同 Release 边车资产内容，不符或锚不可得均拒装不回落，fail-closed）；latest 判新三态（already-latest、localNewer 报告不动、更新），双腿降级守卫由暂存件 --version 预检承载，命中按 localNewer 收束（rc=0 不装不回落）；自替换三步舞加固（exe 旁更新锁 create_new 加 pid 活性判加 mtime 超窗兜底加陈旧收割加 drop 清锁、死 pid 残件收割且 .old 救援件 exe 缺位保留、unix 暂存件 chmod 755、入位后 --version 自证五次重试期望版仅从 semver 形 tag 推导（dev 等回退可跑判）、证败或入位败回滚并复核终态、回滚受阻报自救路径、证毕清理下载残件）；管理方布局让位（exe 同目录 ark-managed 落痕，或本 exe 路径即用户面 bin 符号链接且目标目录有同落痕，即拦自更新 CTA 走 ark；用户自建便利链接不误拦，落痕生产者契约派 ark 侧批次）。update.rs 单测净增七件（八新一退役）加 cli 集成 update_version_output_parses，205 加 40 绿、clippy 无新增、aidoc strict 0、实弹缺省镜像腿 already-latest 加 dev 腿降级收束 rc=0。
- **hook 注册自愈哨兵 v2.5.0**（issue #31、REQ-014；REQ-015 立评估）：状态栏 hst 段在 state=unknown 时探注册面（按 agent 定位配置文件查 `hst-state` 干 stem 标记，覆盖 Unix `.sh` 与 Windows `.ps1`/`.cmd` 注册形态，四家在册名才探），缺位升格 `no-hook!` 红色可见告警；节流窗（每 agent 1 小时，stamp=`~/.hst/state/.hookcheck-<agent>` 首创建独占、读侧 `[IO.File]` 直取 mtime 且失败 fail-closed）到期 best-effort 调 `hst hook init` 自愈（幂等仅 hook 面保语义不保字节；hst 不在 PATH 或失败静默，渲染 rc 恒 0）；哨兵自含于生成脚本零常驻进程零新依赖。行为测试装配点播种注册标记保住「无状态但在册显示 unknown」既有语义（seed_registration 加 run_statusline_opts 免播种变体）。statusline.rs 单测四件新（形锁两件加行为两件含节流回归锁，heal 腿 stub PATH 隔离，CI 可移植），本机实弹闭环两轮：摘净注册渲染出红字 no-hook! 同次渲染自动愈合（标记 0 回 8）二轮回 unknown；评审 F1（Unix pwsh dotfile Hidden 打穿节流）加 F2（.sh 字面标记在 Windows 注册形态恒失配）收口后，PATH 无 hst 节流窗内 stamp 不动、清 stamp 真 PATH 一渲染即愈。根因侧（注册形态迁移防覆盖）立 REQ-015 评估不静默丢。issue list 同型修（#52，browse-rs 立单的上游档案）：默认 limit 20 升 100（服务端上限，旧默认静默截断致 open 集超限后旧条目隐形）、返回条数打满钳制后 limit 时 stderr 出截断提示（指向 --status 收窄或网页面）、help 写明默认值与 count 语义（count 为返回条数非在册总数）。#53 keyset 翻页随卷（参照 browse 0167e8c）：list 加 --before <id> 游标（取该 id 之前更旧一页、非法值 400 透传归因、不带 before 旧形回执不变），带 before 响应含 has_more 权威翻页信号（kv 与 json 信封透出、饱和提示以它判定），饱和提示补 --before 出口；list_issues 回执类型升 IssueListPage（签名加 before），库面无外部消费者按 minor 记（检索在案，评审 G4 裁）。

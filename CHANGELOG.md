...
- **selfupdate 家族统一标准对齐 v2.4.0**（总台家族自更新统一标准轮 2026-09-18 用户裁「采纳推广」，权威 = browse-rs REQ-005 加 build-release 公共契约第六节；ADR-0008 替代 ADR-0004、REQ-013）：缺省读序翻向（`HST_MIRROR` 未设 = 默认基址 env.ohmygh.com 镜像段优先，stable 通道落 stable 滚动段，任一步网络类失败整对回落 GitHub 官方不回环；设值 = 基址覆盖同读序、空串 = 全关；kv 标记 fallback-default 形退役换 default-first 形，D48 回退守卫函数随结构化不回环退役）；GitHub 官方腿补 digest 锚硬校验（API digest 归一优先、缺省回落同 Release 边车资产内容，不符拒装不回落，锚不可得才 warn skip-verify）；latest 判新三态（already-latest、localNewer 报告不动、更新），镜像 stable 腿降级守卫由暂存件 --version 预检承载；自替换三步舞加固（exe 旁更新锁 create_new 加 pid 活性判加陈旧收割加 drop 清锁、死 pid 残件收割、unix 暂存件 chmod 755、入位后 --version 自证五次重试期望版从 release tag 注入、证败或入位败回滚并复核终态、回滚受阻报自救路径）；管理方布局让位（exe 同目录 ark-managed 落痕或用户面 bin 符号链接指向本 exe 即拦自更新 CTA 走 ark，落痕生产者契约派 ark 侧批次）。update.rs 单测净增五件（六新一退役）加 cli 集成 update_version_output_parses，203 加 40 绿、clippy 无新增、aidoc strict 0、实弹缺省镜像腿 already-latest。

...
- **hook 注册自愈哨兵 v2.5.0**（issue #31、REQ-014；REQ-015 立评估）：状态栏 hst 段在 state=unknown 时探注册面（按 agent 定位配置文件查 `hst-state.sh` 标记，四家在册名才探），缺位升格 `no-hook!` 红色可见告警；节流窗（每 agent 1 小时，stamp=`~/.hst/state/.hookcheck-<agent>` 先落防自旋）到期 best-effort 调 `hst hook init` 自愈（幂等仅 hook 面；hst 不在 PATH 或失败静默，渲染 rc 恒 0）；哨兵自含于生成脚本零常驻进程零新依赖。行为测试装配点播种注册标记保住「无状态但在册显示 unknown」既有语义（seed_registration 加 run_statusline_opts 免播种变体）。statusline.rs 单测三件新（形锁两件加行为一件，CI 可移植），本机实弹闭环：摘净注册渲染出红字 no-hook!、同次渲染自动愈合（标记 0 回 8）、二轮回 unknown。根因侧（注册形态迁移防覆盖）立 REQ-015 评估不静默丢。

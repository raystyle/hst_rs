...
- **hook 注册自愈哨兵（v2.5.0 主体，issue #31、REQ-014；e7d5581 加 8b58cd8 加 073b55a）**：状态栏 hst 段在 state=unknown 时探注册面（按 agent 定位配置文件查 `hst-state` 干 stem 标记，覆盖 Unix `.sh` 与 Windows `.ps1`/`.cmd` 注册形态，四家在册名才探），缺位升格 `no-hook!` 红色可见告警；节流窗（每 agent 1 小时，stamp 首创建独占、读侧 `[IO.File]` 直取 mtime 且失败 fail-closed）到期 best-effort 调 `hst hook init` 自愈（幂等仅 hook 面保语义不保字节；hst 不在 PATH 或失败静默，渲染 rc 恒 0）；哨兵自含于生成脚本零常驻进程零新依赖。单测四件新（形锁两件加行为两件含节流回归锁，heal 腿 stub PATH 隔离 CI 可移植）；实弹闭环两轮含评审 F1（Unix pwsh dotfile Hidden 打穿节流）加 F2（.sh 字面在 Windows 注册形态恒失配）收口；三条已知边界与防复发三家覆盖模拟一触全愈在 REQ-014 在册
- **hook 注册形态迁移评估收口（REQ-015 翻 implemented、REQ-016 立项；52c071e 加 f48b7d9）**：claude 裁定迁 `~/.claude/skills/` 插件位（三证：探针插件无头双钩实发零信任屏、evo-adr 插件 hook 活例证声明形同构、officecli 在载不在册证自管无注册表竞争；enabledPlugins 启停态考据与 statusLine 面待裁入 ADR-0009）；codex/grok/kimi 单源面不迁（REQ-014 哨兵为终态缓解）；状态键超窗静默探针与自愈腿烘绝对路径加关闭开关两小件归 REQ-016
...
- **ledger 账本集成（issue 面切源，REQ-018 加总台 REQ-063 Phase 3）**：`issue new/list/show/close` 与 `artifact publish/attest/promote/list` 命令族原生集成，真源 ledger.ohmygh.com（旧 issues.ohmygh.com 客户端面退役，服务过渡保役）；写入五头 Ed25519 签名道（私钥 env 或密档 `~/.hst/ledger/ed25519.key`，公钥 JWK 常量内置，kid = sha256hex(JWK)），签名基七行换行连同源服务端；issue 族承继家族标准（缺省 100 加 before keyset 加 has_more 权威饱和判定加 count 语义）；`hst ledger keygen` 密钥对管理；家族三件（钳制加饱和加提示）迁 ledger 且提示改指账本网页面；env `HST_LEDGER_URL` 加 `HST_LEDGER_PRIVATE_KEY` 新增。

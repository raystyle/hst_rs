...
- `cross-test.sh`：全平台实弹测试矩阵（REQ-007）——本地构建 linux/mac/windows-gnu 三产物，scp 到 lan-ubuntu 与 lan-linux 与 lan-mac 跑 --version 加 doctor 冒烟；Windows 宿主走 127.0.0.1 回环 powershell 冒烟。用法 `bash .tools/cross-test.sh`。

...
| `HST_LEDGER_PRIVATE_KEY` | 账本签名私钥（hex 32 字节；缺省读 `~/.hst/ledger/ed25519.key`） | 未设走密档 |
...
issue 账本流（真源 ledger.ohmygh.com）：`hst issue new "<标题>" --kind bug --acceptance "<验收条件>"` 开单；`hst issue list` / `hst issue show <n>` 读面；关单与状态推进归 omc 工作台（herdr 委托 omc ledger issue status）。产物共享库：`hst artifact publish <名> --kind experience --digest <sha256:...> --note "<说明>"` 等（--summary 加 --outcome 加 --git-sha 三旗标随 crate v0.1.3 齐）。

...
/// 新增子命令在此补一行，`hst init` 重跑即同步（带生成标记才覆写）。
const COMMAND_MAP: &[(&str, &str)] = &[
    (
        "hst init [--project PATH] [--yolo[=full|partial|off] | --project-yolo[=full|partial|off]] [--pre-trust]",
        "部署 hook/skill 与 yolo 键（幂等；hook 注册用户级常驻 ~/.hst/hooks/ shim，状态按 session 分键，项目旧注册自动退役；yolo 两级显式：--yolo 用户级全机、--project-yolo 项目级覆盖，D28；D33 分级 full/partial/off，off 摘 hst 落键，级别缺省 full）",
    ),
    (
        "hst doctor",
    ...
    (
        "hst agents",
        "检测四家 agent 已装情况（PATH/环境变量/hst 自管根/默认目录四源）",
    ),
    (
...
/// 尾巴 rerun hst init）：前缀双收才能把旧项目生成物认作 ours 继续覆写升级
/// （整串匹配会漏旧尾巴永不覆写，D30 修）。
...
/// 部署的单点写入语义面（细则见 R002 与模块文档）。
/// D49 第 2 轮：单名 hst，无兼容体（write_skill 体参保留供测试注入）。
fn write_skill(path: &Path, generated: &str, report: &mut DeployReport) -> Result<(), String> {
    match std::fs::read_to_string(path) {
        Ok(existing) => {
            if existing == generated {
                report.skipped.push(path.display().to_string());
            } else if existing.contains(SKILL_MARKER)
                || existing.contains(LEGACY_SKILL_MARKER_PREFIX)
                || existing == LEGACY_SKILL_MD
            {
...
/// Skills: `.agents/skills/hst` is the canonical source (D49; the legacy
/// ohmyagents dirs are retired in place). Claude and Grok and Kimi get
/// copies (Claude does not scan .agents/skills, S008).
fn deploy_skills(root: &Path, report: &mut DeployReport) -> Result<(), String> {
    // D49：canonical 名 hst 唯一源；旧牌 ohmyagents 目录幂等退役
    //（第 2 轮裁定直接删除，ours 识别同 write_skill 家族）。
    let source = root.join(".agents").join("skills").join("hst");
    write_skill(&source.join("SKILL.md"), &skill_md(), report)?;
    for target in [".agents", ".claude", ".grok", ".kimi-code"] {
        let base = root.join(target).join("skills");
        write_skill(&base.join("hst").join("SKILL.md"), &skill_md(), report)?;
        retire_legacy_skill_dir(&base.join("ohmyagents"), report);
    }
    Ok(())
}
...
fn deploy_kimi_project(root: &Path, report: &mut DeployReport) -> Result<(), String> {
    // D49：canonical hst（旧牌目录由 deploy_skills 统一退役）。
    let dir = root.join(".kimi-code").join("skills").join("hst");
...
mod tests {
    ...
    fn retire_strips_project_registrations_and_shims() {
        ...
        assert!(shims.is_dir());
        // skills 仍项目级部署。D49：canonical hst 唯一名，旧牌 ours 目录退役。
        assert!(root
            .join(".agents")
            .join("skills")
            .join("hst")
        ...
        assert!(root
            ...
            .join("SKILL.md")
            .exists());
        // 旧牌 ours 目录（带 marker）被退役删除。
        assert!(!root
            .join(".claude")
            .join("skills")
            .join("ohmyagents")
            .exists());
        // 用户手改的旧牌内容（无 marker）不动。
...
mod skill_tests {
    ...
    fn write_skill_fresh_legacy_upgrade_user_owned() {
        ...
        write_skill(&user, &skill_md(), &mut r4).unwrap();
        assert_eq!(std::fs::read_to_string(&user).unwrap(), "我的私货 skill");
        let _ = std::fs::remove_dir_all(&d);
    }

    #[test]
    fn write_skill_upgrades_v05x_marker_tail() {
        // D30 回归钉：v0.5.x 生成物带旧尾巴 marker（rerun oma init），整串
        // 匹配会漏（v0.6.0 实况：永不覆写）；前缀双收后应继续升级。
        let d = tmp("v05x");
        let p = d.join("SKILL.md");
        std::fs::write(
            &p,
        ...
        assert!(body.contains(SKILL_MARKER), "upgraded to current marker");
        assert!(body.contains("hst init"), "command map refreshed");
        assert_eq!(r.wrote.len(), 1);
        // v0.6.0 混合形态（前缀 oma 尾巴 hst）同样双收。
        std::fs::write(
            &p,
            "<!-- generated by oma init; rerun hst init to sync the command map -->\n\n旧正文\n",
        )
        .unwrap();
        let mut r2 = DeployReport::default();
        write_skill(&p, &skill_md(), &mut r2).unwrap();
